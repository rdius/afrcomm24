...
def run_tracker_with_buffering(filename, model, output_queue):
    ...
    buffering_queue = Queue(maxsize=MAX_QUEUE_SIZE * 4)
    ...
    while True:
        ...
        if not local_queue.empty():
            ...
            buffering_queue.put(processed_frame)

        if not buffering_queue.empty():
            output_queue.put(buffering_queue.get())
...
def main():
    st.title("YOLO Object Detection - With Buffering and Real-time Display")
    ...
    if video_file:
        ...
        output_queue = Queue()
        thread = threading.Thread(target=run_tracker_with_buffering, args=(video_path, model, output_queue), daemon=True)
        thread.start()
