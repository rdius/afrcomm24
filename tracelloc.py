...
def run_tracker_with_memory_management(filename, model, output_queue):
...
def main():
    st.title("YOLO Object Detection - With Memory Management")
    ...
    if video_file:
        ...
        output_queue = Queue()
        thread = threading.Thread(target=run_tracker_with_memory_management, args=(video_path, model, output_queue), daemon=True)
        thread.start()
