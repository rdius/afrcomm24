...
def run_tracker_with_threading(filename, model, output_queue):
...
def main():
    st.title("YOLO Object Detection - With Threading")
    video_file = st.sidebar.file_uploader("Upload Video", type=["mp4", "avi", "mov"])
    if video_file:
        model = YOLO("yolov8n.pt")
        output_queue = Queue()
        thread = threading.Thread(target=run_tracker_with_threading, args=(video_file.name, model, output_queue), daemon=True)
        thread.start()
